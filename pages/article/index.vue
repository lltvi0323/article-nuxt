...
<script>
import { getArticles, removeArticle, updateArticle } from "~/graphql/article";

import ArticleForm from "~/components/ArticleForm";

export default {
  fetch({ redirect, route }) {
  //handle when go to URL not have "id" param
  if (!route.params.id) {
    redirect("/article/1");
  }
},
